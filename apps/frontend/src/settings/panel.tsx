...
export const SidePanel = ({ children }: SidePanelProps) => {
    return <Card className="w-full px-4 py-4 rounded-lg lg:w-80 shadow-none border-2 dark:border border-border">{children}</Card>;
};
    ...
    return (
        ...
        >
            ...
            >
                ...
                <i className="text_stagger_animation text-foreground/40 group-hover:text-foreground/55 hover:text-foreground/55 w-6 flex items-center justify-start">{icon}</i>
                <span className="text_stagger_animation text-lg font-semibold text-foreground-muted group-hover:text-foreground">{label}</span>
            </RouterNavLink>
        </li>
    );
};
