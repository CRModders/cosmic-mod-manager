...
export default function MarkdownEditor({
	...
	function toggleTextAtCursorsLine(
		...
		if (atLineStart === true) {
			const firstSelectedLineIndex =
				editorValue.slice(0, selectionStart).lastIndexOf("\n") + 1; // Start index of the first selected line
			...
			const selectedLines =
				`${editorValue.slice(firstSelectedLineIndex, selectionEnd)}`.split(
					"\n",
			...
			if (action === "ADD_FRAGMENT") {
				...
				if (editorValue[selectionStart - 1] === "\n" || selectionStart === 0) {
					setLastSelectionRange([
						selectionStart,
						selectionEnd - charactersDeletedCount,
					]);
				} else {
					setLastSelectionRange([
						selectionStart - text.length,
			...
			setEditorValue(
				`${linesBeforeSelection}${newSelectedLinesText.slice(0, -1)}${textAfterSelection}`,
			);
		} else {
			...
			<div className="flex w-full flex-col items-start justify-center gap-1">
				...
				<div className="flex w-full flex-wrap items-center justify-between gap-x-6 gap-y-1">
							...
							<LinkInsertionModal
								...
								getMarkdownString={(
									...
									if (editorTextarea.current)
										selectedText = getTextareaSelectedText(
											editorTextarea.current,
										);
					...
					<div className="flex items-center justify-center gap-2">
						...
						/>
						<Label
							htmlFor="markdown-editor-preview-toggle-switch"
							className="text-base"
						>
							{t.preview}
						</Label>
						...
						<Textarea
							...
							onKeyDown={(e) => {
								...
								if (pressedKey === "escape")
									return editorTextarea.current?.blur();
...
function Separator() {
	return (
		<span className="hidden h-10 w-[0.1rem] bg-shallow-background lg:flex" />
	);
}

function BtnGroup({ children }: { children: React.ReactNode }) {
	return (
		<div className="flex items-center justify-center gap-x-1.5 gap-y-0.5">
			{children}
		</div>
	);
}
	...
	return (
		<EditorModal
			...
			insertFragmentFunc={() => {
				...
				insertFragmentFunc(
					getMarkdownString(url, urlAltText, false),
					url,
					urlAltText,
				);
		...
		>
			{altTextInputVisible && (
				<div className="flex w-full flex-col items-start justify-center gap-1.5">
					...
					>
						{altTextInputLabel}{" "}
						{isAltTextRequired && (
							<span className="flex h-full items-start justify-center text-accent-foreground">
								*
							</span>
						)}
					</Label>
			...
			<div className="flex w-full flex-col items-start justify-center gap-1.5">
				...
				>
					{url && !urlValidationError ? (
						<MarkdownRenderBox
							text={getMarkdownString(url, urlAltText, true)
								.split(textSeparatorChar)
								.join("")}
						/>
					) : null}
				</div>
