...
const DialogHeader = ({
	...
	<div
		className={cn(
			"flex flex-col gap-1 px-card-surround pb-4 text-left border-b border-b-shallow-background",
			className,
		)}
		{...props}
	/>
...
const DialogFooter = ({
	...
	<div
		className={cn(
			"w-full flex flex-col-reverse sm:flex-row sm:justify-end gap-form-elements gap-y-2",
			className,
		)}
		{...props}
	/>
...
>(({ className, ...props }, ref) => (
	<DialogPrimitive.Title
		ref={ref}
		className={cn(
			"text-lg text-foreground font-bold leading-none tracking-tight pr-7",
			className,
		)}
		{...props}
	/>
...
>(({ className, ...props }, ref) => (
	<DialogPrimitive.Description
		ref={ref}
		className={cn("text-sm text-muted-foreground pr-8", className)}
		{...props}
	/>
));
