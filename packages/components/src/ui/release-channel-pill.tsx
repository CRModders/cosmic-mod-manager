...
export default function ReleaseChannelChip({
	releaseChannel,
	labelClassName,
	className,
}: Props) {
	return (
		...
		>
			...
			<span
				className={cn(
					"text-muted-foreground/90 leading-none font-semibold",
					labelClassName,
				)}
			>
...
export function ReleaseChannelBadge({ releaseChannel, className }: Props) {
	return (
		...
		>
			<ReleaseChannelIcon
				releaseChannel={releaseChannel as VersionReleaseChannel}
				className="w-5 h-5"
			/>
...
export function releaseChannelBackgroundColor(
