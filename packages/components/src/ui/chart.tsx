...
interface ChartLegentContentProps extends Pick<RechartsPrimitive.LegendProps, "payload" | "verticalAlign"> {
    hideIcon?: boolean;
    nameKey?: string;
}
...
function ChartLegendContent({
    ...
    nameKey,
    ref,
}: ChartLegentContentProps & React.ComponentProps<"div">) {
    const { config } = useChart();
        ...
        <div ref={ref} className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
            {payload.map((item) => {
                ...
                return (
                    <div
                        key={item.value}
                        className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-zinc-500 dark:[&>svg]:text-zinc-400")}
                    >
