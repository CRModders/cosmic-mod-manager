...
const getInitialValues = (orgData?: Organisation | null) => {
    return {
        icon: orgData?.icon || "",
        name: orgData?.name || "",
        slug: orgData?.slug || "",
...
const GeneralOrgSettings = () => {
    const { orgData, fetchOrgData } = useContext(orgDataContext);
    const [isLoading, setIsLoading] = useState(false);

    let initialValues = getInitialValues(orgData);
    ...
    const resetForm = (icon: string) => {
        ...
        form.setValue("slug", orgData.slug);
        form.setValue("description", orgData.description || "");
    };
    ...
    const saveSettings = async (values: z.infer<typeof orgSettingsFormSchema>) => {
        ...
        try {
            ...
            const response = await useFetch(`/api/organization/${orgData?.slug}`, {
                method: "PATCH",
                body: formData,
            });
            ...
            await fetchOrgData(result?.slug || orgData?.slug);
...
export const Component = GeneralOrgSettings;

const DeleteOrgDialog = ({ name, slug }: { name: string; slug: string }) => {
    const [isLoading, setIsLoading] = useState(false);
    ...
    const deleteOrg = async () => {
        ...
        setIsLoading(true);
        try {
            const res = await useFetch(`/api/organization/${slug}`, {
                method: "DELETE",
            });
