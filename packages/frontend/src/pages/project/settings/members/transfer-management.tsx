...
interface Props {
    ...
    projectId: string;
    invalidateProjectPageQueries: () => Promise<void>;
}

export const TransferProjectManagementCard = ({ organisations, projectId, invalidateProjectPageQueries }: Props) => {
    ...
    const transferProject = async () => {
        try {
            ...
            const res = await useFetch(`/api/organization/${selectedOrg}/projects`, {
                method: "POST",
...
export const RemoveProjectFromOrg = ({ org, projectId, invalidateProjectPageQueries }: RemoveProjectFromOrgProps) => {
    const [isLoading, setIsLoading] = useState(false);
    const navigate = useNavigate();
    ...
    const removeProjectFromOrg = async () => {
        try {
            ...
            const res = await useFetch(`/api/organization/${org.id}/project/${projectId}`, {
                method: "DELETE",
            });
            const data = await res.json();
