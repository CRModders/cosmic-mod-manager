...
const VersionChangelogs = () => {
    ...
    return (
        <div className="w-full flex flex-col items-start justify-start gap-panel-cards">
            <ChangelogsList projectData={projectData} versionsList={allProjectVersions} />
        </div>
    )
};
    ...
    return (
        <>
            ...
            <Card className="p-5 w-full flex flex-col items-start justify-start">
                {
                    versionsList.slice((activePage - 1) * perPageLimit, activePage * perPageLimit)
                        .map((version) => {
                                    ...
                                    <div className="w-full flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
                                        <div className="flex flex-wrap gap-x-1.5 items-baseline justify-start">
                                            ...
                                            <h2 className="leading-tight">
                                                <Link to={getProjectVersionPagePathname(projectData.type[0], projectData.slug, version.slug)}
                                                    className="text-xl font-bold text-foreground-bright"
                                                >
                                                    {version.title}
                                                </Link>
                                            </h2>
                                            <span>
                                                by <Link to={`/user/${version.author.userName}`} className="text-link-foreground hover:underline">{version.author.userName}</Link>
                                            </span>
                                            <span>
                                                on {formatDate(new Date(version.datePublished), "${month} ${day}, ${year}", true)}
                                            </span>
                                        </div>

                                        {
                                            version.primaryFile?.url ?
                                                <a href={projectFileUrl(version.primaryFile.url)} className={cn(buttonVariants({ variant: "secondary", size: "sm" }))}>
                                                    <DownloadCloud className="w-btn-icon h-btn-icon" />
                                                    Download
                                                </a>
                                                : null
                                        }
...
const ChangelogBar = ({ releaseChannel }: { releaseChannel: VersionReleaseChannel }) => {
    return (
        <>
            <div className={cn("absolute w-1 h-full bg-current top-2 left-2 rounded-full",
                releaseChannel === VersionReleaseChannel.RELEASE ? "text-blue-500 dark:text-blue-400" :
                    releaseChannel === VersionReleaseChannel.BETA ? "text-orange-500 dark:text-orange-400" :
                        releaseChannel === VersionReleaseChannel.ALPHA ? "text-danger-background" : ""
            )}>
